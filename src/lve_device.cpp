...
void LveDevice::stopRendering() {
    stopRenderingMutex.lock();
    LveSync::getInstance()->setStopRendering(true);
    LveSync::getInstance()->getwaitForStopRenderingMutex().lock();
    LveSync::getInstance()->getwaitForStopRenderingMutex().lock();
    vkQueueWaitIdle(graphicsQueue_);
    int x = 1;
  }

void LveDevice::startRendering() {
    LveSync::getInstance()->setStopRendering(false);
    stopRenderingMutex.unlock();
  }
}  // namespace lve